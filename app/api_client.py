...
class APIClient:
    ...
    def fetch_json(self, endpoint: str, query_list=[]):
        ...
        response = requests.get(url, headers=self.headers, json=query_data, timeout=settings.API_TIMEOUT)
    ...
    def get_experiments(self):
        ...
        query_list = []
        return self.fetch_json("experiment", query_list)
    ...
    def get_completed_drift_runs(self, experiment_id: str, start_datetime, end_datetime, data_drift=True, concept_drift=True):
        ...
        query_list = [
            queries.datetime(start=start_datetime, end=end_datetime),
            queries.data_drift(drift=data_drift),
            queries.concept_drift(drift=concept_drift),
